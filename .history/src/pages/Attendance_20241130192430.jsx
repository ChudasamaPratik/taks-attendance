...
const Attendance = ({ onLogout }) => {
    ...
    const [attendanceTypeCounts, setAttendanceTypeCounts] = useState({
        'Present': 0,
        'Leave': 0,
        'Cancel Leave': 0,
        'Extra Day': 0
    ...
    const getAttendanceClass = (type) => {
        switch (type) {
            case 'Present':
                return 'bg-green-500 hover:bg-green-400 text-white hover:text-gray-100';
            case 'Leave':
                return 'bg-indigo-600 hover:bg-indigo-500 text-white hover:text-gray-100';
            case 'Cancel Leave':
                return 'bg-amber-500 hover:bg-amber-400 text-black hover:text-gray-800';
            case 'Extra Day':
                return 'bg-blue-600 hover:bg-blue-500 text-white hover:text-gray-100';
            default:
                return 'bg-gray-500 hover:bg-gray-400 text-white hover:text-gray-100';
        }
    };
    ...
    const fetchAttendanceRecords = async () => {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                const typeCounts = response.data.attendance.reduce((acc, record) => {
                ...
                setAttendanceTypeCounts(typeCounts);
            } else {
    ...
    return (
        <div className="w-full h-screen bg-gray-900 text-gray-100 flex flex-col p-9">
                        ...
                        >
                            ...
                            <div className="mt-2">
                                {Object.entries(attendanceRecords).map(([date, type], index) => {
                                    return (
                                        new Date(date).toDateString() === day.date.toDateString() && (
                                            <p
                                                key={index}
                                                className={`text-white py-2 rounded-lg transition ${getAttendanceClass(type)}`}
                                            >
                                                {type}
                                            </p>
                                        )
                            ...
                            <p className="text-xs text-gray-400 mt-1 truncate">
                                {Object.entries(attendanceRecords).map((data, index) => {
                                    console.log(data);

                                    return (
                                        <span key={index} className="block">
                                            {day.date.toDateString() === new Date(data.date).toDateString() ? notes : ""}
                                        </span>
                                    );
                                })}
                            </p>
                            ...
                            {Object.entries(attendanceRecords).map(([index, record]) => {
                                const recordDate = new Date(record.date).toDateString();
                                return (
                                    recordDate === day.date.toDateString() && (
                                        <div key={record._id} className="space-y-1">
                                            <p className={`text-white py-2 rounded-lg transition ${getAttendanceClass(record.attendanceType)}`}>
            ...
            {/* Add this section somewhere in your return statement */}
            <div className="bg-gray-800 p-4 rounded-lg mt-4">
                <h3 className="text-xl font-bold mb-4 text-gray-100">Attendance Summary</h3>
                <div className="grid grid-cols-4 gap-4">
                    {Object.entries(attendanceTypeCounts).map(([type, count]) => (
                        <div
                            key={type}
                            className={`p-3 rounded-lg text-center ${getAttendanceClass(type)}`}
                        >
