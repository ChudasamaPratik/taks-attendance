...
const Register = ({ onRegister }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (password !== confirmPassword) {
      ...
      toaster.error('Passwords do not match'); // Show toaster error
      return;
    }
    ...
    try {
      ...
      if (err.response && err.response.data) {
        ...
        // Check if detailed errors are available
        if (err.response.data.errors) {
          Object.values(err.response.data.errors).forEach((error) => {
            toaster.error(error); // Show each error as a toast notification
          });
        } else {
          toaster.error(errorMessage); // General error message
        }
      } else {
        const genericError = 'An error occurred. Please try again.';
        setError(genericError);
        toaster.error(genericError); // Show generic error message
      }
    }
  };
